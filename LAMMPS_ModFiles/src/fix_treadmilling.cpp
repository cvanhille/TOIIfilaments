...
void FixTreadmilling::post_integrate()
{
  ...
  if (comm->me == 0) {
    fprintf(lmp->screen, "FIX TREADMILLING RUN: performing post_integrate operations at timestep %lld\n", update->ntimestep);
  }
  ...
  if (comm->me == 0) {
    fprintf(lmp->screen, "  Neighbor list updated for ghost atoms. Now identifying filaments and performing reactions...\n");
  }
  ...
  for (auto& [mol_id, bounds] : filament_bounds) {
    ...
    if (r_on > 0) {
      if (comm->me == 0) {
        fprintf(lmp->screen, "Evaluating growth for filament %d (head tag %d (idx %d), tail tag %d (idx %d), subhead tag %d (idx %d)) at timestep %lld -- p = %g\n", mol_id, tag[hid], hid, tag[tid], tid, tag[shid], shid, update->ntimestep, r_on * dt);
      }
      if (should_happen(r_on)) {
        ...
        if (comm->me == 0) {
          fprintf(lmp->screen, "  Attempting growth for filament %d (head tag %d (idx %d), tail tag %d (idx %d), subhead tag %d (idx %d)) at timestep %lld\n", mol_id, tag[hid], hid, tag[tid], tid, tag[shid], shid, update->ntimestep);
        }
...
void FixTreadmilling::grow_filament(tagint mol_id, int hidx, int shidx)
{
  ...
  image = atom->image[hidx]; // & image;  // get image flags for head to apply PBCs later 
  ...
  bool placed = false;
  if (comm->me == 0) {
    fprintf(lmp->screen, "  Sampling new position for growth of filament %d at timestep %lld -- head pos (%.3f, %.3f, %.3f), subhead pos (%.3f, %.3f, %.3f)\n", mol_id, update->ntimestep, head_pos[0], head_pos[1], head_pos[2], shead_pos[0], shead_pos[1], shead_pos[2]);
  }
  // Displacement
  ...
  dz *= sigma;
  if (comm->me == 0) {
    fprintf(lmp->screen, "  Displacement for new head position for growth of filament %d at timestep %lld: (%.3f, %.3f, %.3f)\n", mol_id, update->ntimestep, dx, dy, dz);
  ...
  if (comm->me == 0) {
    fprintf(lmp->screen, "  Sampled new position (%.3f, %.3f, %.3f) for growth of filament %d at timestep %lld after %d trials\n", new_pos[0], new_pos[1], new_pos[2], mol_id, update->ntimestep, trials);
    fprintf(lmp->screen, "  Sampled wrapped new position (%.3f, %.3f, %.3f) (image: %d) for growth of filament %d at timestep %lld after %d trials\n", wrapped[0], wrapped[1], wrapped[2], image, mol_id, update->ntimestep, trials);
  }
  ...
  if (is_local(wrapped)) {
    fprintf(lmp->screen, "    Creating new head for filament %d at timestep %lld with tag %d at position (%.3f, %.3f, %.3f) on processor %d\n", mol_id, update->ntimestep, new_tag, wrapped[0], wrapped[1], wrapped[2], comm->me);
    ...
    create_particle(new_pos, new_tag, ptype, mol_id, 1, image_local);
  ...
  if (comm->me == 0) {
    fprintf(lmp->screen, "  Created %d new particles for growth of filament %d at timestep %lld\n", natoms, mol_id, update->ntimestep);
  }
  ...
  if (comm->me == 0) {
    fprintf(lmp->screen, "  Updated global atom count to %lld after growth of filament %d at timestep %lld\n", atom->natoms, mol_id, update->ntimestep);
  }
  natomsloc = 0;
  ...
  if (comm->me == 0) {
    fprintf(lmp->screen, "  Comm forwarded for growth of filament %d at timestep %lld\n", mol_id, update->ntimestep);
  }
  ...
  if (comm->me == 0) {
    fprintf(lmp->screen, "  Rebuilt neighbor list for growth of filament %d at timestep %lld\n", mol_id, update->ntimestep);
  }
  ...
  fprintf(lmp->screen, "  New particle for growth of filament %d at timestep %lld has tag %d and local index %d on processor %d -- filpos = %d, birth_step = %f\n", mol_id, update->ntimestep, new_tag, new_idx, own_proc, filpos[new_idx], birth_step[new_idx]);
